import "./About.css";
import { datesArrToString } from "../../util/StringFormats";
import {work, education, skills} from "./content.js";
import AccordionDiv from "../../components/AccordionDiv/AccordionDiv";
import IconEntry from "../../components/IconEntry/IconEntry";
import ContentBox from "../../components/ContentBox/ContentBox";

function DatedEntry(props) {
    const {title, location, dates, description, list, hiddenList} = props.data;

    return (
        <div>
            <h4 className="moveOver"><strong>{title}, </strong></h4>
            <h4 className="moveOver">{location}</h4>
            <p className="date">{datesArrToString(dates)}</p>
            {description && <p>{description}</p>}
            {list && <ul>{list.map(duty => <li>{duty}</li>)}</ul>}
            {hiddenList && <AccordionDiv title={hiddenList.title}>
            <ul>
                {hiddenList.content.map(entry => <li><strong>{entry.heading}: </strong>{entry.details}</li>)}
            </ul>
            </AccordionDiv>}
        </div>
    );
}

const mediaIcons = {
    gmail: "data:image/svg+xml;base64,",
    linkedin: "data:image/svg+xml;base64,"
};

function MediaLink(props) {
    return (
        <button className="mediaLink" onClick={() => window.open(props.link, "_blank")}>
            <img src={mediaIcons[props.icon]} alt={props.icon}/>
            <p>{props.text}</p>
        </button>
    )
}

export default function About() {
    return (
        <div>
            <ContentBox>
                <h1>Contact</h1>
                <hr />
                <p>
                    Since this is a public site, I am limiting the information available here. If you prefer a method of contact not listed here, please send me
                    an email and I would be happy to provide contact on other platforms.
                </p>
                <MediaLink icon="gmail" text="alaevens@ualberta.ca" link="mailto:alaevens@ualberta.ca"/>
                <MediaLink icon="linkedin" text="Alexander Laevens" link="https://www.linkedin.com/in/alexander-laevens-50a993238/"/>
            </ContentBox>

            <ContentBox>
                <h1>Education</h1>
                <hr />
                {education.map(school => {
                    return (
                        <IconEntry image={school.image} alt={school.title}>
                            <DatedEntry data={school}/>
                        </IconEntry>
                    );
                })}
            </ContentBox>
            
            <ContentBox>
                <h1>Work Experience</h1>
                <hr />
                {work.map(job => {
                    return (
                        <IconEntry image={job.image} alt={job.title}>
                            <DatedEntry data={job} />
                        </IconEntry>
                    );
                })}
            </ContentBox>

            <ContentBox>
                <h1>Technical Skills</h1>
                <hr />
                {skills.map(skillGroup => {
                    return (
                        <AccordionDiv title={skillGroup.subject}>
                            <ul className="skillList">{skillGroup.content.map(skill => <li>{skill}</li>)}</ul>
                        </AccordionDiv>
                    );
                })}
            </ContentBox>
        </div>
    );
}
